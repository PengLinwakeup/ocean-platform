...
interface ChartStyles {
  ...
  aouDocSymbolShape?: 'circle' | 'square' | 'triangle' | 'diamond';
  aouDocSymbolSize?: number;
  aouDocGridX?: boolean;
  aouDocGridY?: boolean;
  aouDocFitLineWidth?: number;
  aouDocFitLineStyle?: 'solid' | 'dashed' | 'dotted';
  aouDocFitLineColor?: string;
  aouDocAouMin?: number | '';
  aouDocAouMax?: number | '';
  aouDocDocMin?: number | '';
  aouDocDocMax?: number | '';
  aouDocUpperColor?: string;
  aouDocIntermediateColor?: string;
  aouDocDeepColor?: string;
...
export default function OriginPlotter({ processedSamples: originalProcessedSamples, stationCoords, hydroSamples, hydroParameters }: OriginPlotterProps) {
  ...
  // --- T-S section states removed as they are handled in TSPublicationStudio ---
  ...
  const [customSelectedStations, setCustomSelectedStations] = useState<string[]>(() => loadSavedState<string[]>('ocean_customSelectedStations', []));
  ...
  //   useEffect(() => {
  // 
  //     localStorage.setItem('ocean_fitOnlyIntermediate', JSON.stringify(fitOnlyIntermediate));
  // 
  //   }, [fitOnlyIntermediate]);
  ...
  useEffect(() => {
    ...
    localStorage.setItem('ocean_selectedRegionFit', JSON.stringify(selectedRegionFit));
  ...
  useEffect(() => {
    ...
    localStorage.setItem('ocean_customSelectedStations', JSON.stringify(customSelectedStations));
  ...
  const [aouDocSettingsTab, setAouDocSettingsTab] = useState<'data' | 'style'>('data');
  ...
  const [chartStyles, setChartStyles] = useState<ChartStyles>(() => {
    ...
    return {
      ...
      aouDocSymbolShape: 'circle',
      aouDocSymbolSize: 4,
      aouDocGridX: true,
      aouDocGridY: true,
      aouDocFitLineWidth: 2,
      aouDocFitLineStyle: 'dashed',
      aouDocFitLineColor: '#059669',
      aouDocAouMin: '',
      aouDocAouMax: '',
      aouDocDocMin: '',
      aouDocDocMax: '',
      aouDocUpperColor: '#ea580c',
      aouDocIntermediateColor: '#059669',
      aouDocDeepColor: '#1d4ed8'
  ...
  const aouDocData = useMemo(() => {
    ...
    return valid.map(s => {
      ...
      // Pass pressure to calculate potential temperature corrected AOU (umol/kg)

      const aouKg = calculateAOU(sal, temp, o2, closest.pressure);
      ...
      // const density = sigma + 1000; // in kg/m3



      // Convert AOU from umol/kg to umol/L: C_L = C_kg * rho / 1000

      // const aouL = parseFloat(aouLVal.toFixed(2));
      ...
      const lat = sc ? sc.latitude : (s.latitude || 0);



      // Group into regions based on longitude
      ...
      return {
        ...
        aou: parseFloat(aouKg.toFixed(2)),
    ...
    }).filter(Boolean) as { station: string; depth: number; doc: number; aou: number; depthGroup: string; longitude: number; latitude: number; region: string }[];

  }, [originalProcessedSamples, findHydroDataForSample, stationCoords]);
  ...
  const aouDocStats = useMemo(() => {
    ...
    if (selectedRegionFit === 'wbc') {
      ...
      data = data.filter(d => d.region === 'cg');
    } else if (selectedRegionFit === 'ebc') {
      data = data.filter(d => d.region === 'ebc');
    } else if (selectedRegionFit === 'custom') {
      data = data.filter(d => customSelectedStations.includes(d.station));
    }

    if (data.length < 2) return null;
    const points = data.map(d => ({ x: d.aou, y: d.doc }));
    return fitCalibrationCurve(points, false);
  }, [aouDocData, selectedRegionFit, customSelectedStations]);
  ...
  }, [aouDocData, aouDocStats, selectedRegionFit, customSelectedStations]);

  const docYDomain = useMemo(() => {
    if (chartStyles.aouDocDocMin !== undefined && chartStyles.aouDocDocMin !== '' && chartStyles.aouDocDocMax !== undefined && chartStyles.aouDocDocMax !== '') {
      return [Number(chartStyles.aouDocDocMin), Number(chartStyles.aouDocDocMax)];
    }
    if (aouDocData.length === 0) return [30, 80];
    const docs = aouDocData.map(d => d.doc);
    const minDoc = Math.min(...docs);
    const maxDoc = Math.max(...docs);
    const calculatedMin = Math.floor(minDoc / 5) * 5;
    const calculatedMax = Math.ceil(maxDoc / 5) * 5;
    return [
      chartStyles.aouDocDocMin !== undefined && chartStyles.aouDocDocMin !== '' ? Number(chartStyles.aouDocDocMin) : calculatedMin,
      chartStyles.aouDocDocMax !== undefined && chartStyles.aouDocDocMax !== '' ? Number(chartStyles.aouDocDocMax) : calculatedMax
    ];
  }, [aouDocData, chartStyles.aouDocDocMin, chartStyles.aouDocDocMax]);

  const aouXDomain = useMemo(() => {
    if (chartStyles.aouDocAouMin !== undefined && chartStyles.aouDocAouMin !== '' && chartStyles.aouDocAouMax !== undefined && chartStyles.aouDocAouMax !== '') {
      return [Number(chartStyles.aouDocAouMin), Number(chartStyles.aouDocAouMax)];
    }
    if (aouDocData.length === 0) return [-50, 250];
    const aous = aouDocData.map(d => d.aou);
    const minAou = Math.min(...aous);
    const maxAou = Math.max(...aous);
    const calculatedMin = Math.floor(minAou / 50) * 50;
    const calculatedMax = Math.ceil(maxAou / 50) * 50;
    return [
      chartStyles.aouDocAouMin !== undefined && chartStyles.aouDocAouMin !== '' ? Number(chartStyles.aouDocAouMin) : calculatedMin,
      chartStyles.aouDocAouMax !== undefined && chartStyles.aouDocAouMax !== '' ? Number(chartStyles.aouDocAouMax) : calculatedMax
    ];
  }, [aouDocData, chartStyles.aouDocAouMin, chartStyles.aouDocAouMax]);
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px', alignItems: 'start' }}>
          <div className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            <h4 style={{ fontSize: '14px', fontWeight: 'bold', borderBottom: '1px solid #e2e8f0', paddingBottom: '6px', margin: 0, color: '#0f172a' }}>
              AOU vs. DOC Respiration Analysis
            </h4>
            
            {/* Sidebar sub-tabs: Data/Fit vs Style */}
            <div style={{ display: 'flex', borderBottom: '1px solid #e2e8f0', marginBottom: '4px' }}>
              <button
                onClick={() => setAouDocSettingsTab('data')}
                className={`tab-btn ${aouDocSettingsTab === 'data' ? 'active' : ''}`}
                style={{
                  flex: 1,
                  padding: '6px 12px',
                  fontSize: '11px',
                  ...
                  background: 'none',
                  border: 'none',
                  borderBottom: aouDocSettingsTab === 'data' ? '2px solid var(--primary-color, #0284c7)' : 'none',
                  color: aouDocSettingsTab === 'data' ? 'var(--primary-color, #0284c7)' : '#64748b',
                  cursor: 'pointer'
                }}
              >
                Data & Fit
              ...
              <button
                onClick={() => setAouDocSettingsTab('style')}
                className={`tab-btn ${aouDocSettingsTab === 'style' ? 'active' : ''}`}
                style={{
                  flex: 1,
                  padding: '6px 12px',
                  fontSize: '11px',
                  ...
                  background: 'none',
                  border: 'none',
                  borderBottom: aouDocSettingsTab === 'style' ? '2px solid var(--primary-color, #0284c7)' : 'none',
                  color: aouDocSettingsTab === 'style' ? 'var(--primary-color, #0284c7)' : '#64748b',
                  cursor: 'pointer'
                }}
              >
                Origin Style
            ...
            {aouDocSettingsTab === 'data' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                <p className="text-xs text-slate-500 leading-relaxed">
                  Apparent Oxygen Utilization (AOU) represents the oxygen consumed during organic matter mineralization. Strictly constrained to intermediate water (200-1000m) for respiration stoichiometry fitting as surface/deep layers do not exhibit respiration stoichiometry.
                </p>
                
                {/* Region Selection */}
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                  <span className="text-xs font-semibold text-slate-700">Fit Region Selection:</span>
                  <select
                    value={selectedRegionFit}
                    onChange={(e) => {
                      const val = e.target.value as any;
                      setSelectedRegionFit(val);
                      if (val === 'custom' && customSelectedStations.length === 0) {
                        // auto-select all unique stations
                        const uniqueSts = Array.from(new Set(aouDocData.map(d => d.station)));
                        setCustomSelectedStations(uniqueSts);
                      }
                    }}
                    className="text-xs"
                    style={{
                  ...
                  >
                    <option value="all">Overall Intermediate Fit (All)</option>
                    <option value="wbc">West Boundary Current (WBC, &lt;55°E)</option>
                    <option value="cg">Central Gyre (CG, 55°E-90°E)</option>
                    <option value="ebc">East Boundary Current (EBC, &gt;90°E)</option>
                    <option value="custom">Custom Stations Fit (Custom)</option>
                  </select>
                </div>

                {/* Custom Station Checkbox List */}
                {selectedRegionFit === 'custom' && (
                  <div style={{
                    backgroundColor: 'var(--bg-secondary)',
                    padding: '8px 12px',
                    borderRadius: '8px',
                    ...
                    <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0', paddingBottom: '4px', marginBottom: '2px' }}>
                      <span style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b' }}>Select Stations:</span>
                      <div style={{ display: 'flex', gap: '6px' }}>
                        <button
                          onClick={() => setCustomSelectedStations(Array.from(new Set(aouDocData.map(d => d.station))))}
                          style={{ background: 'none', border: 'none', padding: 0, fontSize: '9px', color: '#0284c7', cursor: 'pointer' }}
                        >
                          Select All
                        </button>
                        <button
                          onClick={() => setCustomSelectedStations([])}
                          style={{ background: 'none', border: 'none', padding: 0, fontSize: '9px', color: '#64748b', cursor: 'pointer' }}
                        >
                          Clear
                        </button>
                      </div>
                    </div>
                    {Array.from(new Set(aouDocData.map(d => d.station))).sort().map(st => {
                      const count = aouDocData.filter(d => d.station === st).length;
                      return (
                        <label key={st} style={{ display: 'flex', alignItems: 'center', gap: '6px', fontSize: '11px', color: '#334155', cursor: 'pointer' }}>
                          <input
                            type="checkbox"
                            checked={customSelectedStations.includes(st)}
                            onChange={(e) => {
                              if (e.target.checked) {
                                setCustomSelectedStations([...customSelectedStations, st]);
                              } else {
                                setCustomSelectedStations(customSelectedStations.filter(s => s !== st));
                              }
                            }}
                            style={{ cursor: 'pointer' }}
                          />
                          <span>{st} ({count} pts)</span>
                        </label>
                      );
                    })}
                  </div>
                ...
                {aouDocStats ? (
                  <div style={{ backgroundColor: 'var(--bg-secondary)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                    <span className="text-xs font-bold text-sky-800 block mb-1">
                      {selectedRegionFit === 'all' && "🔘 Overall Intermediate Fit:"}
                      {selectedRegionFit === 'wbc' && "🔘 WBC Intermediate Fit:"}
                      {selectedRegionFit === 'cg' && "🔘 CG Intermediate Fit:"}
                      {selectedRegionFit === 'ebc' && "🔘 EBC Intermediate Fit:"}
                      {selectedRegionFit === 'custom' && "🔘 Custom Intermediate Fit:"}
                    </span>
                    <div className="space-y-1 text-xs text-sky-700 font-medium">
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>Fit Equation:</span>
                        <strong>DOC = {aouDocStats.slope.toFixed(4)}*AOU + {aouDocStats.intercept.toFixed(1)}</strong>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>O₂ Contribution (Redfield 1.3):</span>
                        <strong>{(Math.abs(aouDocStats.slope) * 1.3 * 100).toFixed(1)}%</strong>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>O₂ Contribution (A&S 1.45):</span>
                        <strong>{(Math.abs(aouDocStats.slope) * 1.45 * 100).toFixed(1)}%</strong>
                      </div>
                      <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                        <span>R-squared (R²):</span>
                        <strong>{aouDocStats.rsq.toFixed(4)}</strong>
                      </div>
                    </div>
                  </div>
                ) : (
                  <div style={{ backgroundColor: 'var(--bg-secondary)', padding: '8px', borderRadius: '8px', border: '1px solid var(--border-color)', fontSize: '11px', color: '#e11d48', textAlign: 'center' }}>
                    Insufficient intermediate data points to fit.
                  </div>
                )}
              </div>
            )}

            {aouDocSettingsTab === 'style' && (
              <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                {/* Global Font family */}
                <div className="input-group" style={{ marginBottom: 0 }}>
                  <label className="input-label" style={{ fontSize: '10px' }}>Global Font Family</label>
                  <select className="input-field" style={{ fontSize: '11px', padding: '4px' }} value={chartStyles.fontFamily} onChange={e => {
                    const font = e.target.value;
                    setChartStyles(prev => ({ ...prev, fontFamily: font }));
                  }}>
                    <option value="'Times New Roman', Times, serif">Times New Roman (Academic)</option>
                ...
                <div className="grid-2" style={{ gap: '8px' }}>
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label" style={{ fontSize: '10px' }}>Symbol Shape</label>
                    <select className="input-field" style={{ padding: '4px', fontSize: '11px' }} value={chartStyles.aouDocSymbolShape || 'circle'} onChange={e => setChartStyles(prev => ({ ...prev, aouDocSymbolShape: e.target.value as any }))}>
                  ...
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label" style={{ fontSize: '10px' }}>Symbol Size</label>
                    <input type="range" min="3" max="8" step="0.5" className="w-full" value={chartStyles.aouDocSymbolSize ?? 4} onChange={e => setChartStyles(prev => ({ ...prev, aouDocSymbolSize: parseFloat(e.target.value) }))} />
                  </div>
                </div>

                {/* Fit Line Customization */}
                <div className="grid-2" style={{ gap: '8px' }}>
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label" style={{ fontSize: '10px' }}>Fit Line Style</label>
                    <select className="input-field" style={{ padding: '4px', fontSize: '11px' }} value={chartStyles.aouDocFitLineStyle || 'dashed'} onChange={e => setChartStyles(prev => ({ ...prev, aouDocFitLineStyle: e.target.value as any }))}>
                  ...
                  <div className="input-group" style={{ marginBottom: 0 }}>
                    <label className="input-label" style={{ fontSize: '10px' }}>Fit Line Width</label>
                    <input type="range" min="1" max="4" step="0.5" className="w-full" value={chartStyles.aouDocFitLineWidth ?? 2} onChange={e => setChartStyles(prev => ({ ...prev, aouDocFitLineWidth: parseFloat(e.target.value) }))} />
                  </div>
                </div>

                {/* Fit Line Color Picker */}
                <div className="input-group" style={{ marginBottom: 0 }}>
                  <label className="input-label" style={{ fontSize: '10px' }}>Fit Line Color</label>
                  <input
                    type="color"
                    className="w-full"
                    style={{ height: '24px', padding: 0, border: '1px solid var(--border-color)', cursor: 'pointer', borderRadius: '4px' }}
                    value={chartStyles.aouDocFitLineColor || '#059669'}
                    onChange={e => setChartStyles(prev => ({ ...prev, aouDocFitLineColor: e.target.value }))}
                  />
                </div>
                ...
                <div style={{ display: 'flex', gap: '16px', marginTop: '4px' }}>
                  <label className="flex items-center gap-2" style={{ cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>
                    <input type="checkbox" checked={chartStyles.aouDocGridX !== false} onChange={e => setChartStyles(prev => ({ ...prev, aouDocGridX: e.target.checked }))} />
                    <span>Gridlines X</span>
                  </label>
                  <label className="flex items-center gap-2" style={{ cursor: 'pointer', fontSize: '11px', fontWeight: 'bold' }}>
                    <input type="checkbox" checked={chartStyles.aouDocGridY !== false} onChange={e => setChartStyles(prev => ({ ...prev, aouDocGridY: e.target.checked }))} />
                    <span>Gridlines Y</span>
                  </label>
                </div>

                {/* Axis Ranges Customization */}
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '8px', marginTop: '4px' }}>
                  <span className="text-xs font-semibold text-slate-700 block mb-2">Custom Axis Ranges (Origin style)</span>
                  
                  {/* AOU Range */}
                  <div className="grid-2" style={{ gap: '8px', marginBottom: '8px' }}>
                    <div className="input-group" style={{ marginBottom: 0 }}>
                      <label className="input-label" style={{ fontSize: '10px' }}>AOU Min (µmol/kg)</label>
                      <input
                        type="number"
                        className="input-field"
                        style={{ padding: '4px', fontSize: '11px' }}
                        placeholder="Auto"
                        value={chartStyles.aouDocAouMin ?? ''}
                        onChange={e => setChartStyles(prev => ({ ...prev, aouDocAouMin: e.target.value === '' ? '' : parseFloat(e.target.value) }))}
                      />
                    </div>
                    <div className="input-group" style={{ marginBottom: 0 }}>
                      <label className="input-label" style={{ fontSize: '10px' }}>AOU Max (µmol/kg)</label>
                      <input
                        type="number"
                        className="input-field"
                        style={{ padding: '4px', fontSize: '11px' }}
                        placeholder="Auto"
                        value={chartStyles.aouDocAouMax ?? ''}
                        onChange={e => setChartStyles(prev => ({ ...prev, aouDocAouMax: e.target.value === '' ? '' : parseFloat(e.target.value) }))}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid-2" style={{ gap: '8px' }}>
                    <div className="input-group" style={{ marginBottom: 0 }}>
                      <label className="input-label" style={{ fontSize: '10px' }}>DOC Min (µmol/L)</label>
                      <input
                        type="number"
                        className="input-field"
                        style={{ padding: '4px', fontSize: '11px' }}
                        placeholder="Auto"
                        value={chartStyles.aouDocDocMin ?? ''}
                        onChange={e => setChartStyles(prev => ({ ...prev, aouDocDocMin: e.target.value === '' ? '' : parseFloat(e.target.value) }))}
                      />
                    </div>
                    <div className="input-group" style={{ marginBottom: 0 }}>
                      <label className="input-label" style={{ fontSize: '10px' }}>DOC Max (µmol/L)</label>
                      <input
                        type="number"
                        className="input-field"
                        style={{ padding: '4px', fontSize: '11px' }}
                        placeholder="Auto"
                        value={chartStyles.aouDocDocMax ?? ''}
                        onChange={e => setChartStyles(prev => ({ ...prev, aouDocDocMax: e.target.value === '' ? '' : parseFloat(e.target.value) }))}
                      />
                    </div>
                  </div>
                </div>
                ...
                {/* Layer Colors */}
                <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '8px', marginTop: '4px' }}>
                  <span className="text-xs font-semibold text-slate-700 block mb-2">Layer Marker Colors</span>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                      <span className="text-xs text-slate-600">Upper (&lt;200m):</span>
                      <input type="color" style={{ width: '40px', height: '18px', border: 'none', cursor: 'pointer' }} value={chartStyles.aouDocUpperColor || '#ea580c'} onChange={e => setChartStyles(prev => ({ ...prev, aouDocUpperColor: e.target.value }))} />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                      <span className="text-xs text-slate-600">Intermediate (200-1000m):</span>
                      <input type="color" style={{ width: '40px', height: '18px', border: 'none', cursor: 'pointer' }} value={chartStyles.aouDocIntermediateColor || '#059669'} onChange={e => setChartStyles(prev => ({ ...prev, aouDocIntermediateColor: e.target.value }))} />
                    </div>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '8px' }}>
                      <span className="text-xs text-slate-600">Deep (&gt;1000m):</span>
                      <input type="color" style={{ width: '40px', height: '18px', border: 'none', cursor: 'pointer' }} value={chartStyles.aouDocDeepColor || '#1d4ed8'} onChange={e => setChartStyles(prev => ({ ...prev, aouDocDeepColor: e.target.value }))} />
                    </div>
                  </div>
                </div>
              </div>
            ...
            <div style={{ borderTop: '1px solid #f1f5f9', paddingTop: '12px', fontSize: '11px', color: '#64748b' }}>
              <span>Successfully matched DOC and AOU points: <strong className="text-slate-800">{aouDocData.length}</strong></span>
            </div>
          </div>
 
          <div className="card" style={{ display: 'flex', flexDirection: 'column', margin: 0 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '20px' }}>
              <h3 className="card-title" style={{ margin: 0, fontFamily: chartStyles.fontFamily }}>
                DOC vs. AOU Relationship in the Southern Indian Ocean
              </h3>
            </div>
            <div style={{ width: '100%', height: '500px', background: '#ffffff', borderRadius: '8px', padding: '10px' }}>
              {aouDocData.length === 0 ? (
                <div style={{ height: '100%', display: 'flex', justifyContent: 'center', alignItems: 'center', color: '#94a3b8', fontFamily: chartStyles.fontFamily }}>
                  No matching station & depth AOU-DOC points found.
                </div>
              ) : (
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 30, bottom: 30, left: 20 }} style={{ fontFamily: chartStyles.fontFamily }}>
                    <CartesianGrid strokeDasharray="3 3" stroke={chartStyles.gridStroke || '#f1f5f9'} horizontal={chartStyles.aouDocGridY !== false} vertical={chartStyles.aouDocGridX !== false} />
                    <XAxis
                      type="number"
                      dataKey="aou"
                      name="AOU"
                      unit=" µmol/kg"
                      domain={aouXDomain}
                      tick={{ fontSize: chartStyles.fontSizeAxisTick || 10, fill: '#64748b', fontFamily: chartStyles.fontFamily }}
                      label={{ value: 'Apparent Oxygen Utilization (AOU) [µmol/kg]', position: 'insideBottom', offset: -15, fill: '#334155', fontSize: 12, fontWeight: 'bold', fontFamily: chartStyles.fontFamily }}
                    />
                    <YAxis
                      type="number"
                      dataKey="doc"
                      ...
                      label={{ value: 'Dissolved Organic Carbon (DOC) [µmol/L]', angle: -90, position: 'insideLeft', offset: 0, fill: '#334155', fontSize: 12, fontWeight: 'bold', fontFamily: chartStyles.fontFamily }}
                    />
                    <Tooltip
                      cursor={{ strokeDasharray: '3 3' }}
                        ...
                        if (active && payload && payload.length) {
                          ...
                          return (
                            <div style={{
                              ...
                              fontFamily: chartStyles.fontFamily || 'system-ui, sans-serif'
                            }}>
                              <div style={{ fontWeight: 'bold', borderBottom: '1px solid rgba(255,255,255,0.2)', paddingBottom: '2px', marginBottom: '4px', color: '#38bdf8' }}>
                                Station: {data.station}
                              </div>
                              <div>Depth: {data.depth} m</div>
                              <div>AOU: <strong style={{ color: '#fbbf24' }}>{data.aou !== undefined ? data.aou.toFixed(2) : 'N/A'}</strong> µmol/kg</div>
                              <div>DOC: <strong style={{ color: '#fbbf24' }}>{data.doc !== undefined ? data.doc.toFixed(2) : 'N/A'}</strong> µmol/L</div>
                            </div>
                          );
                        }
                        return null;
                    ...
                    />
                    <Legend verticalAlign="top" height={36} style={{ fontFamily: chartStyles.fontFamily, fontSize: 11 }} iconType="circle" />
                    
                    {/* Intermediate Water Fit Line (Selected Region) */}
                    {overallLine.length > 0 && (
                      <Scatter
                        name={
                          selectedRegionFit === 'wbc' ? "WBC Intermediate Fit" :
                          selectedRegionFit === 'cg' ? "CG Intermediate Fit" :
                          selectedRegionFit === 'ebc' ? "EBC Intermediate Fit" :
                          selectedRegionFit === 'custom' ? "Custom Intermediate Fit" :
                          "Overall Intermediate Fit"
                        }
                        data={overallLine}
                        fill="none"
                        line={{
                          stroke: chartStyles.aouDocFitLineColor || '#059669',
                          strokeWidth: chartStyles.aouDocFitLineWidth ?? 2,
                          strokeDasharray: chartStyles.aouDocFitLineStyle === 'solid' ? '0' :
                                           chartStyles.aouDocFitLineStyle === 'dashed' ? '4 4' : '2 2'
                        }}
                        shape={<g />}
                      />
                    )}
                    ...
                    <Scatter
                      ...
                      shape={(props: any) => {
                        const { cx, cy, payload } = props;
                        const size = (chartStyles.aouDocSymbolSize || 4) * 2;
                        
                        let isSelectedRegion = true;
                        if (selectedRegionFit === 'wbc') {
                          isSelectedRegion = payload && payload.region === 'wbc';
                        } else if (selectedRegionFit === 'cg') {
                          isSelectedRegion = payload && payload.region === 'cg';
                        } else if (selectedRegionFit === 'ebc') {
                          isSelectedRegion = payload && payload.region === 'ebc';
                        } else if (selectedRegionFit === 'custom') {
                          isSelectedRegion = payload && customSelectedStations.includes(payload.station);
                        }

                        const fill = isSelectedRegion ? (chartStyles.aouDocUpperColor || '#ea580c') : '#cbd5e1';
                        const stroke = isSelectedRegion ? '#ffffff' : '#cbd5e1';
                        const strokeWidth = isSelectedRegion ? 0.75 : 0.5;
                        const shapeType = chartStyles.aouDocSymbolShape || 'circle';
                        const opacity = isSelectedRegion ? 1.0 : 0.2;

                        return (
                          <g opacity={opacity}>
                            {renderCustomPointShape(cx, cy, size, fill, stroke, strokeWidth, shapeType)}
                          </g>
                        );
                      }}
                    />
                    <Scatter
                      ...
                      shape={(props: any) => {
                        ...
                        const fill = isSelectedRegion ? (chartStyles.aouDocIntermediateColor || '#059669') : '#cbd5e1';
                        const stroke = isSelectedRegion ? '#ffffff' : '#cbd5e1';
                        const strokeWidth = isSelectedRegion ? 0.75 : 0.5;
                        const shapeType = chartStyles.aouDocSymbolShape || 'circle';
                        const opacity = isSelectedRegion ? 1.0 : 0.2;
                    ...
                    <Scatter
                      ...
                      shape={(props: any) => {
                        ...
                        const fill = isSelectedRegion ? (chartStyles.aouDocDeepColor || '#1d4ed8') : '#cbd5e1';
                        const stroke = isSelectedRegion ? '#ffffff' : '#cbd5e1';
                        const strokeWidth = isSelectedRegion ? 0.75 : 0.5;
                        const shapeType = chartStyles.aouDocSymbolShape || 'circle';
                        const opacity = isSelectedRegion ? 1.0 : 0.2;
