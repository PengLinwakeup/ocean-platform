...
export default function OriginPlotter({ processedSamples: originalProcessedSamples, stationCoords, hydroSamples, hydroParameters }: OriginPlotterProps) {
  ...
  const aouDocStats = useMemo(() => {
    let data = aouDocData;
    if (fitOnlyIntermediate) {
      data = data.filter(d => d.depthGroup === 'Intermediate (200-1000m)' && d.aou > 0);
    }
  ...
  }, [aouDocData, fitOnlyIntermediate, selectedRegionFit, customSelectedStations]);

  const upperStats = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Upper (<200m)');
    if (data.length < 2) return null;
    const points = data.map(d => ({ x: d.aou, y: d.doc }));
  ...
  const intermediateStats = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Intermediate (200-1000m)');
    if (data.length < 2) return null;
    const points = data.map(d => ({ x: d.aou, y: d.doc }));
    return fitCalibrationCurve(points, false);
  }, [aouDocData]);
  ...
  const deepStats = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Deep (>1000m)');
    if (data.length < 2) return null;
    const points = data.map(d => ({ x: d.aou, y: d.doc }));
    return fitCalibrationCurve(points, false);
  }, [aouDocData]);
  ...
  }, [aouDocData, aouDocStats, fitOnlyIntermediate, selectedRegionFit, customSelectedStations]);

  const upperLine = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Upper (<200m)');
    if (data.length < 2 || !upperStats) return [];
    const aous = data.map(d => d.aou);
    const minAou = Math.min(...aous);
    const maxAou = Math.max(...aous);
    return [
      { aou: minAou, doc: upperStats.slope * minAou + upperStats.intercept },
      { aou: maxAou, doc: upperStats.slope * maxAou + upperStats.intercept }
    ];
  }, [aouDocData, upperStats]);

  const intermediateLine = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Intermediate (200-1000m)');
    if (data.length < 2 || !intermediateStats) return [];
    const aous = data.map(d => d.aou);
    const minAou = Math.min(...aous);
    const maxAou = Math.max(...aous);
    return [
      { aou: minAou, doc: intermediateStats.slope * minAou + intermediateStats.intercept },
      { aou: maxAou, doc: intermediateStats.slope * maxAou + intermediateStats.intercept }
    ];
  }, [aouDocData, intermediateStats]);

  const deepLine = useMemo(() => {
    const data = aouDocData.filter(d => d.depthGroup === 'Deep (>1000m)');
    if (data.length < 2 || !deepStats) return [];
    const aous = data.map(d => d.aou);
    const minAou = Math.min(...aous);
    const maxAou = Math.max(...aous);
    return [
      { aou: minAou, doc: deepStats.slope * minAou + deepStats.intercept },
      { aou: maxAou, doc: deepStats.slope * maxAou + deepStats.intercept }
    ];
  }, [aouDocData, deepStats]);
  ...
  const docYDomain = useMemo(() => {
    ...
    return [Math.floor(minDoc / 5) * 5, Math.ceil(maxDoc / 5) * 5];
  }, [aouDocData]);
  ...
  const aouXDomain = useMemo(() => {
    ...
    const minAou = Math.min(...aous);
    const maxAou = Math.max(...aous);
    return [Math.floor(minAou / 50) * 50, Math.ceil(maxAou / 50) * 50];
  }, [aouDocData]);
      ...
      {visSubTab === 'aouDocPlot' && (
        <div style={{ display: 'grid', gridTemplateColumns: '320px 1fr', gap: '24px', alignItems: 'start' }}>
          <div className="card" style={{ padding: '20px', display: 'flex', flexDirection: 'column', gap: '16px' }}>
            ...
            <p className="text-xs text-slate-500 leading-relaxed">
              表观耗氧量 (AOU) 反应了水团生物地球化学的老化与有机物矿化消耗氧气的程度。
            </p>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
              
              {/* Region Selection */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                <span className="text-xs font-semibold text-slate-700">拟合海区划分：</span>
                <select
                ...
                >
                  <option value="all">全海区整体拟合 (All)</option>
                  <option value="wbc">西边界流区 (WBC, &lt;55°E)</option>
                  <option value="cg">中央环流区 (CG, 55°E-90°E)</option>
                  <option value="ebc">东边界流区 (EBC, &gt;90°E)</option>
                  <option value="custom">自定义站位筛选拟合 (Custom)</option>
                </select>
              </div>
              ...
              {selectedRegionFit === 'custom' && (
                <div style={{
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between', borderBottom: '1px solid #e2e8f0', paddingBottom: '4px', marginBottom: '2px' }}>
                    <span style={{ fontSize: '10px', fontWeight: 'bold', color: '#64748b' }}>选择拟合站位：</span>
              ...
              {/* Intermediate Filter Checkbox */}
              <label style={{ display: 'flex', alignItems: 'center', gap: '8px', fontSize: '12px', color: '#475569', cursor: 'pointer', marginTop: '4px', userSelect: 'none' }}>
                <input
                  type="checkbox"
                  checked={fitOnlyIntermediate}
                  onChange={(e) => setFitOnlyIntermediate(e.target.checked)}
                  style={{ cursor: 'pointer' }}
                />
                <span style={{ fontWeight: 550 }}>仅对中层水团(200-1000m)进行趋势拟合</span>
              </label>

              {upperStats && (
                <div style={{ backgroundColor: 'rgba(234, 88, 12, 0.08)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(234, 88, 12, 0.2)' }}>
                  <span className="text-xs font-bold text-orange-800 block mb-1">🟠 表层水团 (&lt;200m) 拟合：</span>
                  <div className="space-y-1 text-xs text-orange-700 font-medium">
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>拟合公式:</span>
                      <strong>DOC = {upperStats.slope.toFixed(4)}*AOU + {upperStats.intercept.toFixed(1)}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (Redfield 1.3):</span>
                      <strong>{(Math.abs(upperStats.slope) * 1.3 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (A&S 1.45):</span>
                      <strong>{(Math.abs(upperStats.slope) * 1.45 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>决定系数 (R²):</span>
                      <strong>{upperStats.rsq.toFixed(4)}</strong>
                    </div>
                  </div>
                </div>
              )}
              {intermediateStats && (
                <div style={{ backgroundColor: 'rgba(5, 150, 105, 0.08)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(5, 150, 105, 0.2)' }}>
                  <span className="text-xs font-bold text-emerald-800 block mb-1">🟢 中层水团 (200-1000m) 拟合：</span>
                  <div className="space-y-1 text-xs text-emerald-700 font-medium">
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>拟合公式:</span>
                      <strong>DOC = {intermediateStats.slope.toFixed(4)}*AOU + {intermediateStats.intercept.toFixed(1)}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (Redfield 1.3):</span>
                      <strong>{(Math.abs(intermediateStats.slope) * 1.3 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (A&S 1.45):</span>
                      <strong>{(Math.abs(intermediateStats.slope) * 1.45 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>决定系数 (R²):</span>
                      <strong>{intermediateStats.rsq.toFixed(4)}</strong>
                    </div>
                  </div>
                </div>
              )}
              {deepStats && (
                <div style={{ backgroundColor: 'rgba(29, 78, 216, 0.08)', padding: '12px', borderRadius: '8px', border: '1px solid rgba(29, 78, 216, 0.2)' }}>
                  <span className="text-xs font-bold text-blue-800 block mb-1">🔵 深层水团 (&gt;1000m) 拟合：</span>
                  <div className="space-y-1 text-xs text-blue-700 font-medium">
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>拟合公式:</span>
                      <strong>DOC = {deepStats.slope.toFixed(4)}*AOU + {deepStats.intercept.toFixed(1)}</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (Redfield 1.3):</span>
                      <strong>{(Math.abs(deepStats.slope) * 1.3 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>氧气消耗贡献 (A&S 1.45):</span>
                      <strong>{(Math.abs(deepStats.slope) * 1.45 * 100).toFixed(1)}%</strong>
                    </div>
                    <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                      <span>决定系数 (R²):</span>
                      <strong>{deepStats.rsq.toFixed(4)}</strong>
                    </div>
                  </div>
                </div>
              )}
              {aouDocStats ? (
                <div style={{ backgroundColor: 'var(--bg-secondary)', padding: '12px', borderRadius: '8px', border: '1px solid var(--border-color)' }}>
                  <span className="text-xs font-bold text-sky-800 block mb-1">
                    {selectedRegionFit === 'all' && "🔘 全海区整体趋势拟合："}
                    {selectedRegionFit === 'wbc' && "🔘 西边界流区趋势拟合："}
                    {selectedRegionFit === 'cg' && "🔘 中央环流区趋势拟合："}
                    {selectedRegionFit === 'ebc' && "🔘 东边界流区趋势拟合："}
                    {selectedRegionFit === 'custom' && "🔘 自定义选定站位拟合："}
                  </span>
                ...
                <ResponsiveContainer width="100%" height="100%">
                  <ScatterChart margin={{ top: 20, right: 30, bottom: 30, left: 20 }}>
                    ...
                    <XAxis
                      ...
                      domain={aouXDomain}
                      tick={{ fontSize: 10, fill: '#64748b' }}
                      label={{ value: 'Apparent Oxygen Utilization (AOU) [µmol/L]', position: 'insideBottom', offset: -15, fill: '#334155', fontSize: 12, fontWeight: 'bold' }}
                    />
                    <YAxis
                      type="number"
                      dataKey="doc"
                    ...
                    {/* Linear Regression Fit Line for Upper/Surface Water */}
                    {upperLine.length > 0 && (
                      <Scatter
                        name="表层水拟合趋势线"
                        data={upperLine}
                        fill="none"
                        line={{ stroke: '#ea580c', strokeWidth: 2, strokeDasharray: '4 4' }}
                        shape={<g />}
                      />
                    )}
 
                    {/* Linear Regression Fit Line for Intermediate Water */}
                    {intermediateLine.length > 0 && (
                      <Scatter
                        name="中层水拟合趋势线"
                        data={intermediateLine}
                        fill="none"
                        line={{ stroke: '#059669', strokeWidth: 2, strokeDasharray: '4 4' }}
                        shape={<g />}
                      />
                    )}
 
                    {/* Linear Regression Fit Line for Deep Water */}
                    {deepLine.length > 0 && (
                      <Scatter
                        name="深层水拟合趋势线"
                        data={deepLine}
                        fill="none"
                        line={{ stroke: '#1d4ed8', strokeWidth: 2, strokeDasharray: '4 4' }}
                        shape={<g />}
                      />
                    )}

                    {/* Overall/Intermediate Fit Line */}
                    {overallLine.length > 0 && (
                      <Scatter
                        name={
                          selectedRegionFit === 'wbc' ? "西边界流区拟合线" :
                          selectedRegionFit === 'cg' ? "中央环流区拟合线" :
                          selectedRegionFit === 'ebc' ? "东边界流区拟合线" :
                          selectedRegionFit === 'custom' ? "自定义选定站位拟合线" :
                          fitOnlyIntermediate ? "中层整体拟合趋势线" : "全深度混合拟合趋势线"
                        }
                        data={overallLine}
                        fill="none"
                        line={{ stroke: '#0284c7', strokeWidth: 2, strokeDasharray: '2 2' }}
                        shape={<g />}
                      />
                    )}
