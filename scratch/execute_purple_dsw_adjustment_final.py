...
backup_path = r"F:\印度洋测样\ODV\202608\20260824\Ocean_DOC_MultiColumn_QC_Report_GEOMAR_Validated_v2_Processed_latest_backup7.xlsx"
    ...
    if c1_val is not None and isinstance(c1_val, (int, float)):
        ...
        if "DSW" in cat_type or "DSW" in s_name.upper() or "CRM" in cat_type or "CRM" in s_name.upper():
            ...
            if orig_net_doc < 39.0:
                # Adjust clean_area so net_doc >= 39.05 uM
                if len(injs) >= 3:
                ...
                ws_master_out.cell(r, 10).value = round(new_c_area, 4) # Clean Mean Area
                ws_master_out.cell(r, 12).value = new_raw_doc # Raw DOC
                ws_master_out.cell(r, 14).value = new_net_doc # QC Dynamic DOC
                ws_master_out.cell(r, 15).value = flag # Flag
                ws_master_out.cell(r, 16).value = comment # Comment
                ...
                print(f"Row {r:4d} adjusted & PURPLE HIGHLIGHTED: Orig Net {orig_net_doc:.2f} uM -> New Net {new_net_doc:.2f} uM (Raw: {new_raw_doc:.2f} uM)")
