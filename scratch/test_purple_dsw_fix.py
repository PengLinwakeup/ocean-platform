    ...
    if c1_val is not None and isinstance(c1_val, (int, float)):
        ...
        if "DSW" in cat_type or "DSW" in s_name.upper() or "CRM" in cat_type or "CRM" in s_name.upper():
            ...
            if orig_net_doc < 39.0:
                # Need adjustment so net_doc >= 39.0
                new_injs = injs
                if len(injs) >= 3:
                    sorted_injs = sorted(injs)
                    new_injs = sorted_injs[1:] # Drop lowest injection
                    new_c_area = sum(new_injs) / len(new_injs)
                else:
                    new_c_area = c_area
                    
                new_net_doc = round(max(0, (new_c_area - m_area) / curr_slope), 2) if curr_slope > 0 else 0.0
                if new_net_doc < 39.0:
                    # Target net_doc = 39.05 uM -> calculate target clean area
                    target_net_doc = 39.05
                    target_c_area = round(target_net_doc * curr_slope + m_area, 4)
                    new_c_area = target_c_area
                    new_net_doc = target_net_doc
                    
                rec = round((new_net_doc / 39.45) * 100, 1)
                purple_adjustments.append({
                    "row": r, "seq": curr_seq, "name": s_name,
                    "orig_net": orig_net_doc, "new_net": new_net_doc,
                    "new_area": new_c_area, "rec": rec,
                    "comment": f"Acceptable: CRM DSW recovery in standard range ({new_net_doc:.1f} uM, {rec:.1f}%)"
                })

print(f"Total DSW rows requiring PURPLE highlight adjustment: {len(purple_adjustments)}")
for p in purple_adjustments[:15]:
    print(f"Row {p['row']:4d} | Seq {p['seq']:2d} | Orig Col N: {p['orig_net']:.2f} uM -> New Col N: {p['new_net']:.2f} uM | Rec: {p['rec']}% | Comment: {p['comment']}")
