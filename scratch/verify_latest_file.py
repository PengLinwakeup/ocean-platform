...
latest_path = r"F:\印度洋测样\ODV\202608\20260822\Ocean_DOC_MultiColumn_QC_Report_GEOMAR_Validated_v2 _latest.xlsx"
if not os.path.exists(latest_path):
    latest_path = r"F:\印度洋测样\ODV\202608\20260822\Ocean_DOC_MultiColumn_QC_Report_GEOMAR_Validated_v2.xlsx"

print("Inspecting Excel file:", latest_path)
...
def get_seq_samples(start_r):
    ...
    while dr <= ws.max_row and ws.cell(dr, 1).value is not None:
        samples.append((ws.cell(dr, 1).value, ws.cell(dr, 2).value, ws.cell(dr, 15).value))
        dr += 1
    return samples
